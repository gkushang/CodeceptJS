...
class WebDriver extends Helper {
  constructor(config) {
    ...
    this.customLocatorStrategy = config.customLocatorStrategy;
  ...
  _isCustomLocator(locator) {
    ...
    if (locator.type === 'custom' || locator.custom) {
      if ((typeof this.customLocatorStrategy === 'function')) {
        return true;
      }
      throw new Error(`Custom Locator Strategy isn't defined or is not a function: ${this.customLocatorStrategy}. Pls define as WebDriver helpers configuration: WebDriver: { ...customLocatorStrategy: <function>}`);
    }
    return false;
  }
  ...
  async _startBrowser() {
    ...
    if (this._isCustomLocatorStrategyDefined()) {
      this.debugSection('Weddriver', 'adding custom locator strategy');
      await this.browser.addLocatorStrategy('customlocatorstrategy', this.customLocatorStrategy);
    }
  ...
  _isCustomLocatorStrategyDefined() {
    return typeof this.customLocatorStrategy === 'function';
  }
  ...
  async _locate(locator, smartWait = false) {
    ...
    if (!this.options.smartWait || !smartWait) {
      if (this._isCustomLocator(locator)) {
        return this.browser.custom$$('customlocatorstrategy', this._grabCustomLocator(locator));
      }
...
async function findClickable(locator, locateFn) {
  ...
  if (this._grabCustomLocator(locator)) {
