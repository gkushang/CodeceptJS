...
class WebDriver extends Helper {
  ...
  _isCustomLocator(locator) {
    ...
    if (locatorObj.isCustom()) {
      ...
      throw new Error(`Custom Locator Strategy isn't defined or is not a function: ${locatorObj.type} Please define as WebDriver helpers configuration: WebDriver: { ...customLocatorStrategies: {custom : <function>}}`);
    }
    return false;
  }
  ...
  async _startBrowser() {
    ...
    if (this._isCustomLocatorStrategyDefined()) {
      Object.keys(this.customLocatorStrategies).forEach(async (customLocator) => {
        ...
        await this.browser.addLocatorStrategy(customLocator, locatorFunction);
      });
    }
