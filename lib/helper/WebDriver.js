...
class WebDriver extends Helper {
  ...
  async _locate(locator, smartWait = false) {
    ...
    if (locator.type === 'shadowDom' || locator.shadowDom) {
      const shadowDom = locator.value ? locator.value : locator.shadowDom;

      let els = await (this.browser.$$(shadowDom.host));

      for (let index = 0; index < shadowDom.children.length; index++) {
        if (Array.isArray(els)) {
          els = (await (els[0].shadow$$(shadowDom.children[index])));
        } else {
          els = (await (els.shadow$$(shadowDom.children[index])));
        }
      }

      this.debugSection('Elements', `Found ${els.length} 'shadowDom' elements`);
