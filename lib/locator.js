 ...
 * **Experimental!** Works for WebDriver helper only
 *
 * Checks if `locator` is custom locator.
 *
 * Custom locators are
 * `{ custom: 'recipe-hello' }`
 *
 * @param {{custom: string}} locator
 *
 * @returns {boolean}
 */
function isCustomLocator(locator) {
  const hasCustomProperty = (locator.custom !== undefined) && (Object.keys(locator).length === 1);
